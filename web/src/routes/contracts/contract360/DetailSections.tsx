import { buildReviewFields, computeReviewProgress, indexEvidence } from "../review/reviewViewModel";
...
export function KeyTermsSection({ contract, evidence }: { contract: Contract360Body; evidence: readonly ContractFieldEvidenceBody[] }) {
  ...
  // Same rows and gate the Review screen builds, so the two counts can never disagree (an evidence
  // row outside the correctable set, or a missing field, is not something Review asks about).
  const reviewCount = computeReviewProgress(buildReviewFields(contract, [], indexEvidence(evidence))).blockingCount;
  return (
