...
export interface ApiClient {
  ...
  /**
   * Calls `POST /api/chat/query` (operationId `askContigo`) -- Ask Contigo's routed answer/abstain
   * envelope behind `src/routes/ask/` (ADR-020 screen 7). Same never-throws shape as every other
   * call here: a `400` (blank question, missing tenant header, or a retrieval/answer failure) is a
   * normal, expected outcome the caller renders inline. A `200` with `response.canDetermine ===
   * false` is not a failure either -- it is the screen's own Abstain or unknown-question-fallback
   * state (see `AskContigoResult`'s own doc comment for how to tell the two apart).
   */
