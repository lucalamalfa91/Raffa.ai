...
export interface operations {
  ...
  getDocument: {
    responses: {
      200: {
        content: {
          "application/json": { id: string; contractId: string | null; fileName: string; mimeType: string; documentType: "Msa" | "OrderForm" | "Sow" | "Amendment" | "RenewalLetter" | "Quote" | "Invoice" | "PriceList" | "Nda" | "Dpa" | "Other"; processingStatus: "Uploaded" | "Processing" | "NeedsReview" | "Completed" | "Failed" | "Rejected"; createdAt: string; pageCount: number | null; isPageCountLimited: boolean };
        };
      };
      400: {
        content: {
  ...
  getPortfolio: {
    responses: {
      200: {
        content: {
          "application/json": { items: ({ contractId: string; supplierId: string | null; supplierName: string | null; type: "Msa" | "OrderForm" | "Amendment" | "Sow" | "RenewalLetter" | "Other"; annualSpend: number | null; currency?: string; startDate: string | null; endDate: string | null; renewalDate: string | null; cancellationDeadline: string | null; autoRenewal: boolean; status: string; risk: string | null; fileName?: string | null; documentProcessingStatus?: string | null })[]; page: number; pageSize: number; totalCount: number; processingDocumentCount: number };
        };
      };
      400: {
        content: {
  ...
  getRenewals: {
    responses: {
      200: {
        content: {
          "application/json": { items: ({ contractId: string; supplierId: string | null; supplierName: string | null; status: "Determined" | "NoRenewal" | "CannotDetermine"; contractStatus: string; documentProcessingStatus: string | null; renewalDate: string | null; daysUntilRenewal: number | null; annualSpend: number | null; cancellationDeadline: string | null; daysUntilCancellationDeadline: number | null; autoRenewal: boolean; action: string; savedAction?: { contractId: string; owner: string; status: "NotStarted" | "InProgress" | "Completed"; action: string; updatedAt: string } | null; priority: { contractId: string; totalScore: number; components: { spendWeight: { score: number; explanation: string }; timeUrgency: { score: number; explanation: string }; benchmarkOpportunity: { score: number; explanation: string }; priceIncreaseRisk: { score: number; explanation: string }; contractRisk: { score: number; explanation: string } } } | null; insightCard: { facts: { supplierId: string | null; supplierName: string | null; renewalDate: string | null; daysUntilRenewal: number | null; annualSpend: number | null; cancellationDeadline: string | null; daysUntilCancellationDeadline: number | null }; recommendations: { recommendedAction: string; explanation: string; annualUpliftPercent: number | null; marketPosition: string | null; potentialSavingsRange: string | null } } })[]; totalCount: number };
        };
      };
      400: {
        content: {
