...
Five reply kinds, five layouts in the UI: `answer` (prose, one evidence
card, buttons), `redirect` and `refusal` (warm prose + one CTA), `abstain`
(the accent-left block, only when there is truly nothing to stand on) and
`interview` (ADR-030: one clarifying question with clickable options, or the
consent alert when the option is "search the public web").
