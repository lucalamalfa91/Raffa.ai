...
| 8 | **The create form submitted the country *label*** ("Switzerland") where `POST /api/workspaces` validates the ISO 3166-1 alpha-2 *code* (`CH`) — found by the first `dev` walk of W14-A2 (2026-09-13): *"'Switzerland' is not a supported workspace country. Expected one of: AT, CH, DE, IT."* The pick row also printed the stored value verbatim, so it would have read "· CHF · CH". Fixed in PR #98 (`web/src/routes/signin/WorkspacePickerScreen.tsx`: label = name, value = code; row maps code → name). | Until #98 is on `dev`, W14-A2 cannot pass from the browser (the API half — the `curl` with `"country":"CH"` — already does). Two `main` runs had also skipped `deploy (dev)` on a red `build + test` (fixed in #97), which is why the wave reached `dev` only on 2026-09-13 17:00Z — the first moment this row could be walked at all. |
