...
| 5 | **Bulk whole-tenant reprocess landed in W17.** `reprocess-tenant-documents.yml` (task E20/F02/US02/T01) is now available and re-enqueues every document in the verify-tenant-corpus worklist through `DocumentReprocessService.ReprocessAsync`. For a single document the Admin resubmit path in the product (`Documents → Retry`) still applies. | Run `reprocess-tenant-documents.yml` against the tenant, then `verify-tenant-corpus.yml` to confirm the worklist is empty and `%PDF` is gone. |
