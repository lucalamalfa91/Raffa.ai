...
| GET | `/api/quotes` | The tenant's quotes, newest first (task E19/F02/US01/T01, quote-read-api; parent story us-01-quote-read-api AC-1; ADR-028 D2, wave w16 NW-12); `X-Tenant-Id` header; response `{ items }`, each item `{ id, fileName, mimeType, processingStatus, supplier, currency, geography, purchaseDate, createdAt }` -- the same stored-field set `POST /api/quotes` already echoes on upload, read back via the new `Raffa.Quotes.Application.QuoteQueryService` (modelled on `PortfolioQueryService`/`DocumentQueryService`; returns stored fields and computes nothing); no paging/filters yet |
| GET | `/api/quotes/{id}` | One quote **with its recorded negotiation outcomes embedded, newest first** (task E19/F02/US01/T01, quote-read-api; parent story us-01-quote-read-api AC-2/AC-3/AC-4; ADR-028 D2) -- a quote with none recorded returns an empty `outcomes` array, never 404; `X-Tenant-Id` header; 404 when `{id}` does not name a quote for this tenant (including another tenant's quote -- RLS-backed, zero fields leaked in any form), 400 when it is not a GUID. Response is the same shape as the list row above plus `outcomes: [{ id, originalQuoteTotal, targetPrice, finalPrice, realizedSaving, discountPercent, negotiationDurationDays, leversUsed, capturedAt, savingsOpportunityId }]` -- the same fields `POST /api/negotiations/outcomes` returns on capture, minus the two capture-time-only propagation-attempt fields (`savingsPropagated`/`savingsPropagationError`), which are not stored facts. **The assessment endpoint below is deliberately not overloaded with this** (ADR-028 D2): `POST .../assessment/recalculate` returns that same assessment shape, so carrying the outcome there would make a recalculation appear to re-report a negotiation it never touched -- one extra `GET` on mount is the cheaper cost. **No tenant-wide `GET /api/negotiations/outcomes` exists** -- a bare outcome feed has no caller until NW-57 (W18) and this route does not pre-empt that item's product shape |
