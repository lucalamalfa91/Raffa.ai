...
namespace Contigo.Chat.Tests.Capabilities;

/// <summary>
/// Proves task E13/F08/US01/T01's catalog completeness (story us-01-capability-catalog AC-1/AC-4):
/// every V2 route named in `contigo-v2/ia-v2.md`'s route map (plus the two Documents sub-states
...
public sealed class CapabilityCatalogTests
{
    ...
    [Fact]
    public void FeatureCitation_for_uses_the_contigo_corpus_and_route_as_subtitle_and_href()
    {
        var capability = CapabilityCatalog.Find("savings")!;

        var citation = FeatureCitation.For(capability);

        Assert.Equal("contigo", citation.Corpus);
