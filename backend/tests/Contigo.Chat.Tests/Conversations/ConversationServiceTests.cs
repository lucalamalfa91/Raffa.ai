...
namespace Contigo.Chat.Tests.Conversations;
...
/// <c>Contigo.Documents.Contracts.Tests.DocumentUploadServiceTests</c>/<c>DocumentQueryServiceTests</c>:
...
{
    private const string AppRoleName = "contigo_chat_app";
    private const string AppRolePassword = "contigo_chat_app_test_password";
    ...
    private static AppendConversationMessageRequest ContigoAnswer(
        string markdown, string citationsJson = "[{\"n\":1}]", string modelId = "fixture-answer-v1") =>
        new(ConversationRole.Contigo, ConversationMessageKind.Answer, markdown, citationsJson, "[]", modelId, "answer-v1", "hash-abc");
    ...
    public async Task GetAsync_returns_the_conversation_with_its_messages_in_creation_order()
    {
        ...
        }
        {
            ...
            await service.AppendMessageAsync(
                ...
                ContigoAnswer("Salesforce ends on **15 January 2027** [1]"));
        }
        ...
        Assert.Equal(ConversationRole.Contigo, detail.Messages[1].Role);
