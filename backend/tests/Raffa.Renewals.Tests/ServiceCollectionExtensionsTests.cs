...
/// (mirrors <c>Raffa.Chat.Tests.ServiceCollectionExtensionsTests</c>): each artifact this module
...
/// <c>Raffa.Api.Program</c> / <c>Raffa.Worker.Program</c> actually rely on:
...
/// E03/F01/US02/T02), <see cref="Raffa.Renewals.Application.RenewalThresholdScheduler"/> plus its
...
/// <c>Raffa.Chat.Tests.ServiceCollectionExtensionsTests</c>): <see cref="RenewalEngine"/> is
...
/// shape a future host wiring (<c>Raffa.Api.Program</c> / <c>Raffa.Worker.Program</c>) will
...
/// <c>Raffa.Chat.Tests.ServiceCollectionExtensionsTests</c>): <see cref="RenewalEngine"/> and
...
/// <c>Raffa.Worker.WorkerServiceCollectionExtensions.AddWorkerHost</c> relies on now that it
...
/// <c>Raffa.Api.Tests.DeployableApiTests</c>/<c>Raffa.Worker.Tests.DeployableWorkerTests</c>
...
public sealed class ServiceCollectionExtensionsTests
{
    ...
    public void AddRenewalsModule_resolves_RenewalEngine_and_RenewalThresholdScheduler_with_no_captive_dependency()
    {
        ...
        // RenewalThresholdScheduler needs IAuditWriter (Raffa.Audit's own AddAuditModule job in
    ...
    /// from the same container — <c>Raffa.Api.RenewalsEndpointExtensions</c> takes it as a
    ...
    /// from the same container — <c>Raffa.Api.RenewalsEndpointExtensions</c>' new POST handler
    ...
    /// the same container — <c>Raffa.Worker.Scheduling.RenewalThresholdSchedulerHostedService</c>
    /// and <c>Raffa.Api.RenewalAlertRecomputeService</c> both take it as a constructor/DI
