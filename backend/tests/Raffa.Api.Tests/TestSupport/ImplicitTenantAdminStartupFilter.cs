...
public sealed class ImplicitTenantAdminStartupFilter : IStartupFilter
{
    ...
    public Action<IApplicationBuilder> Configure(Action<IApplicationBuilder> next) => app =>
    {
        app.Use(async (context, nextMiddleware) =>
        {
            ...
            // An explicit X-User-Id equal to this filter's own address is the same thing as none: a test
            // helper with a default caller can name "some member" without seeding anything.
            if (!headers.ContainsKey(UserHeaderName) || headers[UserHeaderName].ToString() == Email)
            {
            ...
            else if (context.RequestServices.GetService<PresentedCallersAsMembersPolicy>() is not null
                && headers.TryGetValue(TenantHeaderName, out var namedTenant)
                && Guid.TryParse(namedTenant.ToString(), out var namedTenantGuid)
                && !string.IsNullOrWhiteSpace(headers[UserHeaderName].ToString()))
            {
                // Opted-in hosts only (see PresentedCallersAsMembersPolicy): the caller the test names
                // is made a member of the tenant it names, so the endpoint under test is reached.
                await EnsureMembershipAsync(context.RequestServices, new TenantId(namedTenantGuid), headers[UserHeaderName].ToString(), WorkspaceRoleName.Admin, context.RequestAborted)
                    .ConfigureAwait(false);
            }
        ...
        IServiceProvider services, TenantId tenant, string email, WorkspaceRoleName role, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // carry the same value there too -- it is what TestUserIdAuthenticationHandler puts on
            // the `oid` claim from this same X-User-Id value.
