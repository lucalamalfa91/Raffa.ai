...
/// <c>Raffa.Benchmark.Fixtures.FixtureBenchmarkAdapter</c>'s own catalog rows — Salesforce Sales
/// Cloud Enterprise, 100 seats, 12-month US/USD (P25/P50/P75 = 1500/1800/2100 per seat/year, sample
/// size 512) — the same "matched quote" stand-in <c>Raffa.Quotes.Tests.MarketAssessmentServiceTests</c>/
/// <c>NegotiationStrategyServiceTests</c> and <c>Raffa.IntegrationTests.R3EndToEndTests</c> already
...
/// <c>Raffa.Quotes.Domain.SkuProductMapping</c> seeded anywhere — every tenant starts with zero
...
internal static class R4ExtractionFixtures
{
    ...
    /// (2,100), so <c>Raffa.Quotes.Application.Assessment.MarketAssessmentCalculator</c> classifies
    ...
    /// <summary>Exact literal <c>Raffa.Benchmark.Contracts.BenchmarkQuery.Term</c> match for the
    /// 12-month Salesforce/Sales Cloud Enterprise catalog row — see
    /// <c>Raffa.Quotes.Application.Assessment.MarketAssessmentQueryBuilder</c>'s own doc comment for
    /// why this is passed through verbatim, never normalized against
    /// <c>Raffa.Quotes.Application.Normalization.QuoteBillingCadence</c>'s own, different,
    ...
    /// <c>Raffa.IntegrationTests.QuoteEndToEndTests</c>/<c>R1EndToEndTests</c>).</summary>
