...
/// (<c>Raffa.Api.ChatEndpointExtensions</c>/<c>ConversationsEndpointExtensions</c>, both
/// delegating into <c>Raffa.Api.AskCopilotService</c>), against a real
/// Postgres+pgvector+RLS Testcontainer (<see cref="R0IntegrationFixture"/>, extended by this task
/// to also migrate Raffa.Chat's and Raffa.Suppliers.Products' own tables — see that fixture's
...
/// though <c>Raffa.AiGateway.Fixtures.FixtureAiGateway</c>'s embeddings are SHA-256 pseudo-vectors
...
/// </summary>
public sealed class AskRaffaRagCrossTenantIsolationTests : IClassFixture<R0IntegrationFixture>
{
    private readonly R0IntegrationFixture _fixture;

    public AskRaffaRagCrossTenantIsolationTests(R0IntegrationFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task Guard_intervention_on_the_in_domain_path_is_recorded_in_the_audit_entry()
    {
        ...
        var client = _fixture.WithWebHostBuilder(builder =>
        {
            builder.ConfigureTestServices(services =>
            {
                ...
                // Raffa.Chat.Tests.TestSupport.RecordingAuditWriter's own doc comment gives).
    ...
    /// Raffa" section) — this test proves the read/answer side, not the write/indexing side,
    /// which <c>Raffa.Documents.Contracts.Tests.EmbeddingRetrievalServiceTests</c> already covers
    /// on its own.
    /// </summary>
    ...
    /// pack item actually has — <c>Raffa.Chat.AbstainGuard.Enforce(AiAnswerResult,
    ...
    /// <c>Raffa.Chat.Tests.TestSupport.RecordingAuditWriter</c> (that project's own doc comment:
    ...
    /// across test projects since <c>Raffa.IntegrationTests</c> does not reference either of the
