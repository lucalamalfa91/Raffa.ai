...
/// real, composed <c>Raffa.Api</c> host (see <see cref="R3IntegrationFixture"/>) and a real,
...
/// included a task that maps a real, extracted <c>Raffa.Documents.Contracts.Domain.Contract</c>'s
...
/// <c>Raffa.Renewals.Application.RenewalActionService.GetActionAsync</c>. This is also this task's
/// own production-code change made provable at all: before task E04/F04/US01/T01,
/// <c>Raffa.Savings.Infrastructure.ServiceCollectionExtensions.AddSavingsModule</c> never called
/// <c>Raffa.Benchmark.ServiceCollectionExtensions.AddBenchmarkModule</c>, so
...
public sealed class R3EndToEndTests : IClassFixture<R3IntegrationFixture>
{
    ...
    public async Task Benchmark_comparison_reports_percentile_target_saving_confidence_and_provenance_for_a_confident_match()
    {
        ...
        // AC-3 "No paid benchmark provider is called": every Raffa.Benchmark.Adapters
