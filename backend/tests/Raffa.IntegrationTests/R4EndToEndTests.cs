...
/// against the real, composed <c>Raffa.Api</c> host (see <see cref="R4IntegrationFixture"/>) and a
...
/// first test to run quote upload (<c>Raffa.IntegrationTests.QuoteEndToEndTests</c>), market
/// assessment (<c>Raffa.Quotes.Tests.MarketAssessmentServiceTests</c>), negotiation strategy
/// (<c>Raffa.Quotes.Tests.NegotiationStrategyServiceTests</c>) and outcome capture/propagation
/// (<c>Raffa.IntegrationTests.NegotiationOutcomePropagationEndToEndTests</c>) as <b>one continuous
...
/// <c>Raffa.Api.QuotesEndpointExtensions.GetAssessmentAsync</c>'s own inline comment.
/// </item>
/// </list>
///
/// <para>
...
/// <c>Raffa.Quotes.Application.Normalization.SkuNormalizationService.NormalizeAsync</c> call
...
/// (<c>Raffa.Api.QuotesEndpointExtensions.RecalculateAssessmentAsync</c> /
/// <c>Raffa.Quotes.Application.Normalization.SkuMappingService</c>) — this test now drives that
/// real endpoint over real HTTP instead, the same "one real host" posture every other step in this
/// chain already takes; <c>Raffa.Quotes.Tests.SkuMappingServiceTests</c> owns the endpoint's own,
