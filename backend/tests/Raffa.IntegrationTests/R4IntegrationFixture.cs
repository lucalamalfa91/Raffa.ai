...
/// against a real, migrated Postgres+pgvector+RLS database and the real <c>Raffa.Api</c>
...
/// <c>Raffa.Benchmark.Fixtures.FixtureBenchmarkAdapter</c> catalog row (task E05/F01/US01/T01's own
...
{
    private const string AppRoleName = "raffa_r4_app";
    private const string AppRolePassword = "raffa_r4_app_test_password";
    ...
    public async Task InitializeAsync()
    {
        ...
        // Raffa.Savings's real schema migrated onto this same instance — same "first fixture to
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        // Task E13/F06/US01/T01 (ask-engine) wires AddMarketModule() into every Raffa.Api host,
        // which makes "market-feed" the container's *default* active Benchmark Service adapter
        // (backend/README.md "Market Intelligence"; Raffa.Market's own ServiceCollectionExtensions
        // doc comment) — but this fixture's own R4ExtractionFixtures scripted quote line is built to
        // match Raffa.Benchmark.Fixtures.FixtureBenchmarkAdapter's own catalog row exactly (P25/P50/
        ...
        // in-process.
        builder.UseSetting(
            $"{Raffa.Benchmark.Configuration.BenchmarkAdapterOptions.SectionName}:" +
            $"{nameof(Raffa.Benchmark.Configuration.BenchmarkAdapterOptions.ActiveAdapter)}",
            "fixture");
