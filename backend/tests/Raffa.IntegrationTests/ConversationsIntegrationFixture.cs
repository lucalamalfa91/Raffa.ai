...
/// `Raffa.Chat`'s own tables (<c>Conversation</c>/<c>ConversationMessage</c>) **and**
/// `Raffa.Audit`'s (<c>audit_event</c>) — `ConversationService` writes a real
...
/// <c>Raffa.Api.Tests.ChatEndpointTests</c>'s own doc comment already relies on; this fixture
...
/// <c>Raffa.Chat.Tests.Conversations.ConversationServiceTests</c>: the Testcontainers bootstrap
...
{
    private const string AppRoleName = "raffa_chat_http_app";
    private const string AppRolePassword = "raffa_chat_http_app_test_password";
