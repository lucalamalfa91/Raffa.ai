...
/// <c>Raffa.Renewals.Tests.RenewalAlertRlsCrossTenantIsolationTests</c> exactly, scoped to this
...
///
/// Lives in <c>Raffa.IntegrationTests</c> rather than <c>Raffa.Suppliers.Products.Tests</c>
/// per this task's own file assignment; unlike the <c>R0</c>-<c>R4</c> suites elsewhere in this
/// project it does not go through <c>Raffa.Api</c>'s <c>WebApplicationFactory&lt;Program&gt;</c>
/// — <c>Raffa.Api</c> does not reference <c>Raffa.Suppliers.Products</c> yet (task
/// E13/F06/US01/T01 wires that in), so this test drives <see cref="SuppliersDbContext"/> directly,
/// the same shape <c>Raffa.Renewals.Tests</c>' own per-module Rls tests already use.
///
...
{
    private const string AppRoleName = "raffa_supplier_app";
    private const string AppRolePassword = "raffa_supplier_app_test_password";
