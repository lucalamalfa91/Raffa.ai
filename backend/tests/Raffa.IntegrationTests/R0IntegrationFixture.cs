...
/// <c>Raffa.Api.Program</c> wires all three (this task). One
...
/// <c>Raffa.Identity.Workspace.Tests.WorkspaceRlsCrossTenantIsolationTests</c> — the
...
{
    private const string AppRoleName = "raffa_r0_app";
    private const string AppRolePassword = "raffa_r0_app_test_password";
    ...
    public async Task InitializeAsync()
    {
        ...
        // and POST /api/conversations/{id}/messages, which AskRaffaRagCrossTenantIsolationTests
        ...
        // only, must also migrate Raffa.Chat's and Raffa.Suppliers.Products' own tables and
        ...
        await using (var db = new AuditDbContext(auditOptions.Options))
        {
            ...
            // same assumption appsettings.Development.json's shared `raffa_dev` connection
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        // present (same syntactically-valid-value approach Raffa.Api.Tests already uses).
