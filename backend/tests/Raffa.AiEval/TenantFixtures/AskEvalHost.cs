...
/// <summary>
/// One running Raffa API host per tenant fixture, wired for a deterministic, network-free
...
/// delegates into <c>Raffa.Api.AskCopilotService.AskAsync</c>, the same entry point
...
internal sealed class AskEvalHost : IAsyncDisposable
{
    ...
    /// <summary><c>Raffa.Api.AskCopilotService</c>'s own <c>AuditResourceType</c> — the marker
    /// that separates the engine's one per-turn row from the conversation store's own rows.</summary>
    private const string AskAuditResourceType = "ask_raffa_v2";
    ...
    private const string GoldenSetCallerId = "golden-set@raffa.test";

    private const string UnreachablePlaceholderConnectionString =
        "Host=localhost;Port=5432;Database=raffa_aieval;Username=raffa;Password=raffa;Include Error Detail=true";
    ...
    /// <c>Raffa.Api.Tests.TestSupport.InMemoryAskEngineFactory.SeedContractAsync</c> already
