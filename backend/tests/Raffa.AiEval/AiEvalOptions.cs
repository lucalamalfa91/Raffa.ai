...
/// registration in place instead of substituting the fixture — <c>Raffa.AiGateway
...
internal static class AiEvalOptions
{
    ...
    /// <c>ConnectionStrings__X</c> convention every deployed Raffa setting already uses.</summary>
    ...
    /// for (<c>backend/tests/Raffa.AiEval/reports/last-run.md</c>, git-ignored).</summary>
    ...
    private static string ResolveProjectDirectory([CallerFilePath] string? thisFile = null) =>
        Path.GetDirectoryName(thisFile)
            ?? throw new InvalidOperationException(
                "Could not resolve the Raffa.AiEval project directory from the compiler-supplied " +
