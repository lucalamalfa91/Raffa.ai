...
public sealed class MarketIngestionServiceTests : IAsyncLifetime
{
    ...
    /// <c>Contigo.Documents.Contracts.Tests.EmbeddingRetrievalServiceTests.StubEmbeddingGateway</c>.
    /// Deliberately not wrapped in <c>Contigo.AiGateway.Logging.LoggingAiGateway</c>: this test
    /// proves <see cref="MarketIngestionService"/>'s own insert/update/unchanged counting, not the
    /// gateway decorator's own audit-writing mechanics (covered by <c>Contigo.AiGateway.Tests</c>).
    /// </summary>
