...
/// <c>Migrations/Scripts/identity-workspace.sql</c> is missing, or stale versus the compiled
...
/// --idempotent` makes under the hood — see <c>Infrastructure.IdentityWorkspaceDbContextFactory</c>,
/// the design-time factory that CLI invocation would use) for a fresh idempotent script and
/// compares it against the checked-in file.
///
...
/// runs anywhere `dotnet test` does, independent of
/// <see cref="IdentityWorkspaceMigrationScriptTests"/>'s own Postgres+pgvector container.
/// </summary>
public sealed class IdentityWorkspaceMigrationScriptStaleCheckTests
{
    ...
    private static string ScriptPath(
        ...
        Path.GetFullPath(Path.Combine(
            ...
            "src", "Contigo.Identity.Workspace", "Migrations", "Scripts",
            "identity-workspace.sql"));
    ...
    public async Task Checked_in_script_exists_and_matches_a_fresh_idempotent_generate()
    {
        ...
        Assert.True(
            ...
            "migrations script --idempotent` from src/Contigo.Identity.Workspace (task " +
            "E09/F01/US01/T01).");
        var checkedIn = await File.ReadAllTextAsync(path);

        var optionsBuilder = new DbContextOptionsBuilder<IdentityWorkspaceDbContext>();
        IdentityWorkspaceDbContextOptions.Configure(optionsBuilder, UnusedConnectionString);
        using var db = new IdentityWorkspaceDbContext(optionsBuilder.Options);
