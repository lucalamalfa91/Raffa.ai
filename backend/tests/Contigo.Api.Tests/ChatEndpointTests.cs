...
/// <c>Contigo.IntegrationTests.AskContigoRagCrossTenantIsolationTests</c> still separately proves
...
public sealed class ChatEndpointTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    /// label is answered entirely by <c>Contigo.Chat.Application.Reply.RedirectReplyBuilder
    /// .GreetingOrOffDomain</c> (that method's own doc comment: "No Contigo.AiGateway call happens
    ...
    /// (<c>Contigo.Chat.Application.AskContigoQueryRouter</c>'s own "renew"/"next N days" keyword
    /// match), routed by <c>Contigo.Chat.Application.Planning.IntentPlanner</c> to
    /// <c>AskIntent.StructuredFact</c> and answered by
    /// <c>Contigo.Api.AskCopilotService.BuildStructuredFactPackAsync</c> — the exact method this
    ...
    /// <c>Contigo.Chat.Application.Reply.CopilotReplyBuilder.BuildCitations</c> would fold straight
