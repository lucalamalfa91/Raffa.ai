...
///
/// Both halves of Ask Contigo's retrieval run against the <b>same physical Postgres database</b> in
/// this test — <see cref="DocumentsContractsDbContext"/>'s own tenant-scoped, RLS-backed
/// <c>embedding</c> table (see <see cref="AskContigoRagCrossTenantIsolationTests"/>) and
...
/// DbContext at all (<c>Contigo.ArchitectureTests.DependencyDirectionTests</c>'s own allow-list:
/// <c>Contigo.Market</c> → <c>[SharedKernel, AiGateway, Benchmark]</c> does not include
/// <c>Contigo.Documents.Contracts</c>, and the reverse direction is never allowed either).
///
/// Lives here — not in <c>Contigo.Market.Tests</c> or <c>Contigo.Documents.Contracts.Tests</c> — per
...
/// <see cref="AskContigoRagCrossTenantIsolationTests"/> already establish. Neither module is wired
/// into <c>Contigo.Api</c>'s <c>Program.cs</c> together yet in a way this project's own
...
/// <c>PgVectorMarketKnowledgeRetrievalTests.SeedAsync</c> already use in <c>Contigo.Market.Tests</c>
...
public sealed class MarketIndexIsolationTests : IAsyncLifetime
{
    ...
    public async Task Tenant_search_never_returns_a_market_note()
    {
        ...
        // Belt-and-suspenders, mirroring AskContigoRagCrossTenantIsolationTests's own
