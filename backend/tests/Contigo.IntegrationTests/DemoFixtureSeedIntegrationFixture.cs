...
/// `contigo_dev`/`contigo_demo`) applies cleanly to a real, migrated
/// Postgres+RLS database and that the real <c>Contigo.Api</c> composition
...
public sealed class DemoFixtureSeedIntegrationFixture : WebApplicationFactory<Program>, IAsyncLifetime
{
    ...
    private const string AppRoleName = "contigo_e10_seed_app";
    private const string AppRolePassword = "contigo_e10_seed_app_test_password";
    ...
    public async Task InitializeAsync()
    {
        ...
        // contigo_dev/contigo_demo — read from disk, never re-typed into this test (so it can
    ...
    /// (<c>backend/tests/Contigo.IntegrationTests/</c>) to the checked-in seed script under
    ...
    /// <c>Contigo.Tenancy.Tests.TenantRlsDeployableScriptCheckTests.ScriptPath</c> already uses, so
