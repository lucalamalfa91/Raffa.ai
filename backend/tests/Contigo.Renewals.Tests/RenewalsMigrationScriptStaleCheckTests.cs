...
/// <c>Migrations/Scripts/renewals.sql</c> is missing, or stale versus the compiled
...
/// --idempotent` makes under the hood — see <c>Infrastructure.RenewalsDbContextFactory</c>, the
...
/// runs anywhere `dotnet test` does, independent of <see cref="RenewalsMigrationScriptTests"/>'s
/// own Postgres+pgvector container.
/// </summary>
public sealed class RenewalsMigrationScriptStaleCheckTests
{
    ...
    private static string ScriptPath(
        ...
        Path.GetFullPath(Path.Combine(
            ...
            "src", "Contigo.Renewals", "Migrations", "Scripts",
            "renewals.sql"));
    ...
    public async Task Checked_in_script_exists_and_matches_a_fresh_idempotent_generate()
    {
        ...
        Assert.True(
            ...
            "migrations script --idempotent` from src/Contigo.Renewals (task E09/F01/US01/T01).");
        var checkedIn = await File.ReadAllTextAsync(path);

        var optionsBuilder = new DbContextOptionsBuilder<RenewalsDbContext>();
        RenewalsDbContextOptions.Configure(optionsBuilder, UnusedConnectionString);
        using var db = new RenewalsDbContext(optionsBuilder.Options);
