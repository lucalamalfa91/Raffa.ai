...
using Contigo.Suppliers.Products.Application;
using Contigo.Suppliers.Products.Domain;
using Contigo.Suppliers.Products.Infrastructure;
using Contigo.Suppliers.Products.Tests.TestSupport;
...
/// separately, in <c>Contigo.IntegrationTests.SupplierCrossTenantIsolationTests</c> (same split
/// <c>Contigo.Renewals.Tests</c> already uses between its own service tests and its own
...
public sealed class SupplierResolverTests : IAsyncLifetime
{
    ...
    public async Task ResolveAsync_reuses_the_row_on_the_second_call_for_an_equivalent_name()
    {
        ...
        Result<Contigo.SharedKernel.Suppliers.SupplierRef> first;
        await using (var db = CreateContext())
        {
            var resolver = new SupplierResolver(db, clock);
