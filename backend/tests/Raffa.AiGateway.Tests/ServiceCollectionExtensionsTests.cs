...
/// <c>Raffa.Chat.Tests.ServiceCollectionExtensionsTests</c> already uses: if
...
public class ServiceCollectionExtensionsTests
{
    ...
    public void AddAiGatewayModule_binds_Endpoint_ProjectName_and_DocumentIntelligenceConnection_from_the_root_AiGateway_section()
    {
        ...
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["AiGateway:Endpoint"] = "https://aisvc-raffa.cognitiveservices.azure.com/",
                ["AiGateway:ProjectName"] = "raffa-dev",
                ["AiGateway:DocumentIntelligenceConnection"] = "conn-docint-raffa-dev",
            })
            .Build();
        services.AddSingleton<IConfiguration>(configuration);
        ...
        Assert.Equal("https://aisvc-raffa.cognitiveservices.azure.com/", options.Endpoint);
        Assert.Equal("raffa-dev", options.ProjectName);
        Assert.Equal("conn-docint-raffa-dev", options.DocumentIntelligenceConnection);
    ...
    /// <summary>Same shape as <c>Raffa.Chat.Tests.ServiceCollectionExtensionsTests.NoOpAuditWriter</c>
    /// — <see cref="Raffa.Audit.Infrastructure.ServiceCollectionExtensions.AddAuditModule"/> is
