...
/// into staged extraction, then indexes the result for Ask Raffa retrieval — so a document
...
/// classify -&gt; extract -&gt; portfolio -&gt; 360 -&gt; Ask Raffa ... works end-to-end").
///
...
/// referencing <c>Raffa.Suppliers.Products</c> — the port lives in SharedKernel and this is the
    ...
    ISupplierResolver? supplierResolver = null)
{
    ...
    /// <c>"Document"</c> literal so a resulting Ask Raffa citation's composite id
    ...
    /// number) under <see cref="DocumentSourceType"/>, so Ask Raffa citations resolve back to
