...
namespace Raffa.Market.Infrastructure.Entities;
...
/// <c>Raffa.Documents.Contracts.Domain.Embedding</c>'s own conventions (vector width, a
/// <see cref="Model"/> column) by agreement, not by reference — this module must not depend on
/// <c>Raffa.Documents.Contracts</c> at all (task's own "Do not touch" list;
/// <c>Raffa.ArchitectureTests.DependencyDirectionTests</c>'s allow-list for
/// <c>Raffa.Market</c> does not include it either).
/// </summary>
public sealed class MarketEmbeddingEntity
{
    /// <summary>Fixed at schema time, same value and rationale as
    /// <c>Raffa.Documents.Contracts.Domain.Embedding.VectorDimensions</c> (ADR-004: "small
    ...
    /// <c>Raffa.AiGateway.AiGatewayConstants.EmbeddingDimensions</c>'s own doc comment states.</summary>
