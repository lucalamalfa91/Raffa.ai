...
/// <c>Raffa.Benchmark.Adapters.IBenchmarkProviderAdapter</c> and
/// <c>Raffa.AiGateway.IAiGateway</c> both let a provider swap stay a registration change, never
/// a call-site change.
///
/// Called <em>only</em> by the ingestion job (R-MKT-03: "the provider is called only by that
/// job; at question time Ask reads Raffa's own store and nothing else") — once T02 adds the
