...
/// <c>Raffa.Insights</c> → <c>[SharedKernel, Benchmark]</c>, pure calculators fed by
...
/// module instead of also touching <c>Raffa.slnx</c> and the architecture allow-list.
///
/// <para>
...
/// <c>Raffa.Renewals.Configuration.PriorityScoreWeightsOptions</c>'s own registration already
/// uses) and <see cref="CriticalityScoreCalculator"/> (which resolves that options singleton as a
/// constructor dependency). <c>Raffa.Insights.Negotiation.PricedLineNegotiationCalculator</c> and
/// <c>Raffa.Insights.Strategy.StrategyPackBuilder</c> are static classes with no configuration or
/// other dependency (mirrors <c>Raffa.Quotes.Application.Strategy.NegotiationStrategyCalculator</c>'s
/// own "pure calculator, no DI registration needed" shape) — nothing to register here for either.
/// No host calls <see cref="AddInsightsModule"/> yet: <c>Raffa.Api.InsightsEndpointExtensions</c>
...
public static class ServiceCollectionExtensions
{
    ...
    /// mirrors <c>Raffa.Renewals.Infrastructure.ServiceCollectionExtensions.AddRenewalsModule</c>.
    /// </summary>
    public static IServiceCollection AddInsightsModule(this IServiceCollection services)
    {
        ...
        // Raffa.Renewals.Infrastructure.ServiceCollectionExtensions's own doc comment on this exact
        // choice).
