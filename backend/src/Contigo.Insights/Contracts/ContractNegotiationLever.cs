...
/// <c>Contigo.Quotes.Application.Strategy.NegotiationLeverType</c> for a contract priced line (task
...
/// one: <c>Contigo.Insights</c> cannot reference <c>Contigo.Quotes</c> (ADR-002; both modules'
/// allow-list is exactly <c>[SharedKernel, Benchmark]</c> —
/// <c>Contigo.ArchitectureTests.DependencyDirectionTests</c>), the same "own enum, same names"
/// treatment <c>Contigo.Renewals.Domain.ContractRiskLevel</c> already gives
/// <c>RiskSeverity</c>.
/// </summary>
...
/// Deliberately not <c>Contigo.Quotes.Application.Strategy.NegotiationLever</c>'s span/page/
/// confidence evidence shape: a <c>Contigo.Benchmark.Contracts.PricedLine</c> carries none of that
...
/// <c>Contigo.Quotes.Application.Strategy.NegotiationLever</c>'s identical "grounded vs generic"
/// distinction).</param>
...
/// <summary>
/// The outcome of <c>Contigo.Insights.Negotiation.PricedLineNegotiationCalculator.Compute</c> for one
/// <c>Contigo.Benchmark.Contracts.PricedLine</c> (task E13/F07/US01/T01; parent story us-01-insights
/// AC-3/AC-4). Bundles both the numeric targets and the levers for that line — the same "one line,
/// one strategy" shape <c>Contigo.Quotes.Application.Strategy.LineNegotiationStrategy</c> already
