...
/// <summary>
/// One contract's raw facts for <c>Contigo.Insights.Criticality.CriticalityScoreCalculator.Calculate</c>
...
/// entities; composition in <c>Contigo.Api</c>") and <c>Contigo.Insights</c>'s own allow-list
/// (<c>[SharedKernel, Benchmark]</c>) mean this record can never carry a real
/// <c>Contigo.Documents.Contracts.Domain.Contract</c>/<c>Contigo.Renewals.Application
/// .PriorityScoreResult</c> — only <c>Contigo.Api</c>, "the one project allowed to reference every
/// module", can build one, the same composition-root pattern
/// <c>Contigo.Renewals.Application.RenewalPriorityInputs</c>/<c>ContractRenewalTerms</c> already
...
/// <c>Contigo.Documents.Contracts.Application.PortfolioListItem.Risk</c>'s own "null = none
...
/// the two consistent — see <c>Contigo.Api.InsightsEndpointExtensions</c>'s own doc comment: no
...
/// <c>Contigo.Api.InsightsEndpointExtensions</c>'s own doc comment for exactly which). Empty, not
...
/// <c>Contigo.Renewals.Application.PriorityScoreResult.TotalScore</c>'s own 0-100 scale (under the
/// product-spec-default <c>PriorityScoreWeightsOptions</c>, five components at a maximum of 20
/// each — <c>Contigo.Renewals.Application.PriorityScoreCalculator.MaxTotalScore</c>) without
/// referencing that type directly — <c>Contigo.Insights</c> cannot reference
/// <c>Contigo.Renewals</c> (ADR-002; allow-list <c>[SharedKernel, Benchmark]</c>). A composition
...
/// <summary>
/// Mirrors <c>Contigo.Documents.Contracts.Domain.RiskSeverity</c>'s four levels plus an explicit
/// <see cref="None"/> for "nothing recorded" — the same "own enum, composition maps 1:1" shape
/// <c>Contigo.Renewals.Domain.ContractRiskLevel</c>'s own doc comment already documents for the
/// identical ADR-002 dependency-direction reason (<c>Contigo.Insights</c> cannot reference
/// <c>Contigo.Documents.Contracts</c>).
/// </summary>
...
/// supplier-name/geography field to build a <c>Contigo.Benchmark.Contracts.BenchmarkQuery</c> from —
/// the same gap <c>Contigo.Documents.Contracts.Application.Contract360Result.Benchmark</c>'s own doc
/// comment already names; see <c>Contigo.Api.InsightsEndpointExtensions</c>).</param>
/// <param name="SavingsOpportunityRangeLow">The conservative end of this contract's aggregated
/// <c>Contigo.Savings.Domain.SavingsOpportunity</c> estimated-savings range (summed across every
...
/// (<c>Contigo.Api.InsightsEndpointExtensions</c>) decides what counts as "critical" for a given
...
/// <param name="Confidence">Contigo's own confidence score in [0, 1] for this fact (Appendix C rule
