...
/// <c>Contigo.Chat.Application.Capabilities.FeatureCitation.ContigoCorpus</c>'s own literal for
...
public static class PackCorpus
{
    ...
    /// <summary>A Contigo feature/capability citation (R-SYS-03) — same value as
    /// <see cref="Capabilities.FeatureCitation.ContigoCorpus"/>.</summary>
    public const string Contigo = "contigo";
    ...
    /// <c>Contigo.Insights.Contracts.InsightsCitationKeys.Calc</c>'s identical "name the
...
///
/// <para>
/// <see cref="Contigo.Chat"/> owns only this DTO shape and <see cref="PackBudget"/> — assembling
/// real <see cref="PackItem"/> rows from the tenant store, the market feed and the calculators is
/// the composition root's job (<c>Contigo.Api.AskCopilotService</c>), the one project allowed to
/// reference every module (ADR-002).
/// </para>
/// </summary>
/// <param name="CitationKey">Stable key the model cites (as an entry of
/// <c>Contigo.AiGateway.Contracts.AiAnswerResult.CitationKeys</c>) and the answer's own inline
...
/// <see cref="PackCorpus.Contigo"/>/<see cref="PackCorpus.Calc"/> items.</param>
...
/// <see cref="PackCorpus.Tenant"/>, a capability route for <see cref="PackCorpus.Contigo"/>
...
/// tenant items, <c>Contigo.Market.Contracts.MarketProvenance.Label</c>'s own "representative
/// market data · mock feed · updated ..." for market items (R-MKT-04), a fixed "Contigo feature"
/// label for contigo items, "deterministic calculator" for calc items. Never omitted — every pack
