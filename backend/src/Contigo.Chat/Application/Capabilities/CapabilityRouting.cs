...
/// <c>AskContigoQueryRouter</c> (no dependency on anything, yet still an injected instance rather
...
{
    /// <summary>ADR-024 empty-state copy (`contigo-v2/markup.html`, Portfolio's `kbOff` block):
    ...
    /// "Quote check →" — `contigo-v2/app.jsx`'s capability-list reply actions, verbatim per
