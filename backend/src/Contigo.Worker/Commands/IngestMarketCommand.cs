...
namespace Contigo.Worker.Commands;

/// <summary>
/// The Worker's operator ingestion entry point (task E13/F02/US01/T02, task objective, verbatim):
/// <c>dotnet run --project backend/src/Contigo.Worker -- ingest-market --feed
...
/// mirroring <c>Contigo.Worker.Scheduling.RenewalThresholdSchedulerHostedService</c>'s own
/// internal visibility.
///
...
/// version *tag* the provider already knows about (<see cref="Contigo.Market.IMarketIntelligenceProvider.GetDealsAsync"/>'s
/// own doc comment); the checked-in
/// <see cref="Contigo.Market.Mock.MockMarketIntelligenceProvider"/> (task E13/F02/US01/T01) reads
...
internal static class IngestMarketCommand
{
        ...
        IServiceProvider hostServices, string[] args, CancellationToken cancellationToken = default)
    {
        ...
        // Contigo.Worker.Scheduling.RenewalThresholdSchedulerHostedService already uses for its own
