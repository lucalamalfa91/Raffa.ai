...
/// <c>Contigo.Quotes.Domain.QuoteLine</c> + <c>Contigo.Quotes.Application.Assessment.LineTargetSaving</c>
/// together already carry for one quote line, so a <c>Contigo.Documents.Contracts.Domain.ContractLineItem</c>
/// can be assessed the same deterministic way without either module depending on the other's entity.
///
/// <para>
/// <b>Why this DTO lives in <c>Contigo.Benchmark</c>, not <c>Contigo.Insights</c></b>: ADR-024's
/// module map puts <c>Contigo.Insights</c> at <c>[SharedKernel, Benchmark]</c> — the same allow-list
/// <c>Contigo.Quotes</c> already has (<c>Contigo.ArchitectureTests.DependencyDirectionTests
...
/// scope — see the parent task's own "Do not touch" list). <c>Contigo.Benchmark</c> is the one
...
/// <c>Contigo.Quotes.Domain.QuoteLine.Description</c> / <c>Contigo.Documents.Contracts.Domain
...
/// <summary>
/// The one piece of negotiation-target arithmetic <c>Contigo.Quotes.Application.Strategy
/// .NegotiationStrategyCalculator</c> (quote lines) and <c>Contigo.Insights.Negotiation
...
/// each module's own, independently-implemented arithmetic — <c>Contigo.Quotes.Application.Assessment
