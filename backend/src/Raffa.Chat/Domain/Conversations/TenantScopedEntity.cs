...
/// <c>Raffa.Documents.Contracts.Domain.TenantScopedEntity</c>'s identical shape, not a shared
/// reference to it: ADR-002's allow-list for <c>Raffa.Chat</c> is exactly
/// <c>[SharedKernel, AiGateway]</c> (<c>Raffa.ArchitectureTests.DependencyDirectionTests</c>) —
/// this module may not reference <c>Raffa.Documents.Contracts</c> at all, so the base type is
/// copied rather than shared, the same way <c>Raffa.Audit.Domain.TenantScopedEntity</c> already
/// is its own copy.
/// </summary>
