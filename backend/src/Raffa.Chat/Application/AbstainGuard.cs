...
/// (Appendix C rule 6, the same determinism convention <see cref="AskRaffaQueryRouter"/> and
...
public sealed class AbstainGuard
{
    ...
    /// "existing AbstainGuard, extended to keys and to market/raffa corpora" — a pack item may be
    /// <see cref="PackCorpus.Tenant"/>, <see cref="PackCorpus.Market"/>,
    /// <see cref="PackCorpus.Raffa"/> or <see cref="PackCorpus.Calc"/>; every corpus is an equally
    ...
    /// (<c>Raffa.AiGateway.Contracts.AiAnswerRequest.PackJson</c>) rather than a bare evidence
    /// list.</param>
...
/// <c>Raffa.AiGateway.Logging.LoggingAiGateway</c> already draws for its own audit rows.
/// </summary>
