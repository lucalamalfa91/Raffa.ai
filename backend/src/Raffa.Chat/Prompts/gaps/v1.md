...
One agent behind ADR-031: before a fresh Ask turn is answered, the capability investigator reads
the message beside everything Raffa can do today — the screens of `CapabilityCatalog`, the Ask
abilities below and the operations `CapabilityGapCatalog` already knows it cannot perform — and
decides whether the user asks for a feature Raffa does not have. The prompt below is the agent's
...
- `question` / `supported`: nothing — the turn is answered as before.
- `known-gap`: the catalog entry named by `knownGapKey` (unless the entry's own veto matches) is
  answered exactly like a regex-matched gap (ADR-030).
- `gap` (confidence at or above `Chat:GapInvestigation:MinConfidence`): every feature text is
  ...
  supplier names and markup removed); the turn becomes a `discovered:<slug>` gap with the honest
  preface, the nearest screen, the alternative questions and the offer to propose the feature.
  The proposal is a GitHub issue labelled `awaiting-approval`; a maintainer approves it.
