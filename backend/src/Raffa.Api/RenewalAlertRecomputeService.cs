...
/// <c>Raffa.Documents.Contracts</c> (to re-read the just-corrected <c>Contract</c> row) and
/// <c>Raffa.Renewals</c> (to recompute its alerts) — ADR-002's dependency-direction rule
/// (<c>Raffa.ArchitectureTests.DependencyDirectionTests</c>) allows neither module to reference the
/// other, only <c>Raffa.Api</c>, the composition root, sees every module at once (backend/README.md's
...
/// <c>Raffa.ArchitectureTests.DependencyDirectionTests.Host_must_not_contain_domain_types</c>, the
...
/// <c>Raffa.Documents.Contracts.Application.ContractCorrectionResult</c> (id/version/corrected
...
/// <see cref="Raffa.Api.RenewalsEndpointExtensions"/>'s own <c>ComputePriority</c> already
