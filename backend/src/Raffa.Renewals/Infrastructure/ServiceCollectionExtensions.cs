...
namespace Raffa.Renewals.Infrastructure;
...
/// real caller is <c>Raffa.Worker.WorkerServiceCollectionExtensions.AddWorkerHost</c>, which now
...
/// real caller is <c>Raffa.Api.Program</c>, which now calls this method and maps
/// <c>GET /api/renewals</c> (<c>Raffa.Api.RenewalsEndpointExtensions</c>).</item>
...
/// caller: <c>Raffa.Api.RenewalsEndpointExtensions</c>'s new <c>GET /api/renewals/{id}/priority</c>
/// route.</item>
...
/// <c>Raffa.Worker.Scheduling.RenewalThresholdSchedulerHostedService</c> (creates alerts from every
/// tick's raised events) and <c>Raffa.Api.RenewalAlertRecomputeService</c> (recomputes on `PATCH
/// /api/contracts/{id}`).</item>
/// </list>
///
/// Every "wiring lands with the first real caller" gap this list used to describe is now closed —
/// <c>Raffa.Chat.Infrastructure.ServiceCollectionExtensions</c>'s own doc comment describes the
/// same sequencing for that module. <c>Raffa.Worker.csproj</c> still only carries a
/// <c>ProjectReference</c> to <c>Raffa.Renewals.csproj</c> in anticipation of
...
/// <c>Raffa.Api.csproj</c>/<c>Raffa.Worker.csproj</c> already carry a <c>ProjectReference</c> to
/// <c>Raffa.Renewals.csproj</c> in anticipation of it, the same "wiring lands with the first real
/// caller" sequencing <c>Raffa.Chat.Infrastructure.ServiceCollectionExtensions</c>'s own doc
...
/// host endpoint takes a dependency on it yet — <c>Raffa.Api.csproj</c>/<c>Raffa.Worker.csproj</c>
/// already carry a <c>ProjectReference</c> to <c>Raffa.Renewals.csproj</c> in anticipation of it,
/// the same "wiring lands with the first real caller" sequencing
/// <c>Raffa.Chat.Infrastructure.ServiceCollectionExtensions</c>'s own doc comment describes for
...
/// <see cref="RenewalEngine"/> directly yet, but <c>Raffa.Worker.WorkerServiceCollectionExtensions
...
/// first real caller" sequencing <c>Raffa.Chat.Infrastructure.ServiceCollectionExtensions</c>'s
...
/// and is the first real caller — <c>Raffa.Api.Program</c> now calls
/// <see cref="AddRenewalsModule"/> and maps <c>GET /api/renewals</c>
/// (<c>Raffa.Api.RenewalsEndpointExtensions</c>), the same "wiring lands with the first real
/// caller" sequencing <c>Raffa.Chat.Infrastructure.ServiceCollectionExtensions</c>'s own doc
/// comment describes for that module. <c>Raffa.Worker.csproj</c> still only carries a
/// <c>ProjectReference</c> to <c>Raffa.Renewals.csproj</c> in anticipation — no worker job calls
...
/// <c>Raffa.Renewals.Application.RenewalOpportunity</c>'s own doc comment named ("no task has
/// given Raffa.Renewals a DbContext yet"). This method's signature therefore now takes a
...
/// (<c>Raffa.Audit.Infrastructure.ServiceCollectionExtensions.AddAuditModule</c>,
/// <c>Raffa.Documents.Contracts.Infrastructure.ServiceCollectionExtensions
/// .AddDocumentsContractsModule</c>) — both existing callers (<c>Raffa.Api.Program</c>,
/// <c>Raffa.Worker.WorkerServiceCollectionExtensions.AddWorkerHost</c>) and every
...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddRenewalsModule(this IServiceCollection services, string connectionString)
    {
        ...
        // Raffa.Chat.Infrastructure.ServiceCollectionExtensions.AddChatModule /
        ...
        // AddXxxModule already picks for its services (see Raffa.Chat's own doc comment on this
        ...
        // Raffa.AiGateway.ServiceCollectionExtensions.AddAiGatewayModule already uses for its own
        ...
        // dependency, and Raffa.Audit's own AuditWriter is registered Scoped (bound to a Scoped
        ...
        // calls this method must also call Raffa.Audit's AddAuditModule for this to resolve —
        // the exact same landmine Raffa.Worker.WorkerServiceCollectionExtensions.AddWorkerHost's
        // own doc comment already flags for Raffa.Documents.Contracts's DocumentUploadService.
        ...
        // any host that calls this method must also call Raffa.Audit's AddAuditModule for that
        // to resolve, the same landmine this method's own doc comment already flags for
        // RenewalThresholdScheduler (both Raffa.Api.Program and
        // Raffa.Worker.WorkerServiceCollectionExtensions.AddWorkerHost already do).
