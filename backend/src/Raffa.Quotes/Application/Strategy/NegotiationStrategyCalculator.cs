...
namespace Raffa.Quotes.Application.Strategy;
...
///
/// <para>
/// <b>Why this stays inside <c>Raffa.Quotes</c>, never calling <c>Raffa.AiGateway</c></b>:
/// <c>Raffa.ArchitectureTests.DependencyDirectionTests</c>' allowed-reference set for this module
...
/// themselves. <c>Raffa.AiGateway.Fixtures.FixtureAiGateway.AnswerAsync</c> would today only echo
...
/// <c>Raffa.Insights.Negotiation.PricedLineNegotiationCalculator</c> (contract priced lines) must
/// reproduce bit-for-bit for a <c>Raffa.Benchmark.Contracts.PricedLine</c> to "yield opening
/// target, acceptable range, walk-away and levers exactly as a quote line does". That shared step
/// lives in <c>Raffa.Benchmark</c>, not <c>Raffa.Insights</c>: <c>Raffa.Quotes</c>' own
/// allow-list (<c>Raffa.ArchitectureTests.DependencyDirectionTests</c>) is exactly
/// <c>[SharedKernel, Benchmark]</c> — it cannot reference <c>Raffa.Insights</c> — so the shared
...
/// <see cref="LineTargetSaving"/>/<see cref="Raffa.Quotes.Domain.QuoteLine"/> in still produces the
/// same <see cref="LineNegotiationStrategy"/> out, so every existing
/// <c>Raffa.Quotes.Tests.NegotiationStrategyCalculatorTests</c> assertion holds unchanged (decimal
...
public static class NegotiationStrategyCalculator
{
    ...
    /// fact: <c>Raffa.Quotes.Application.Normalization.QuoteLineNormalizationService</c> derives
