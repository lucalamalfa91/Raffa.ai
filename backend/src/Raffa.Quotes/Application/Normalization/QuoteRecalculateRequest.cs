...
/// to <see cref="SkuMappingService"/> rather than <c>Raffa.Api</c> — same reason
/// <c>Raffa.Documents.Contracts.Application.ContractCorrectionRequest</c>/<c>Raffa.Quotes
/// .Application.Outcome.NegotiationOutcomeCaptureRequest</c> live next to their own services (see
/// either type's own doc comment): <c>Raffa.ArchitectureTests.DependencyDirectionTests
/// .Host_must_not_contain_domain_types</c> only inspects the <c>Raffa.Api</c>/<c>Raffa.Worker</c>
