...
/// <see cref="SkuMappingService.RecalculateAsync"/>, one entry per <see cref="Raffa.Quotes.Domain
/// .SkuProductMapping"/> a person is creating or correcting.
///
/// <see cref="Sku"/> is the raw, as-extracted SKU text a person saw on an
/// <see cref="Raffa.Quotes.Domain.SkuMatchStatus.Unmatched"/> line (the same raw text
/// <see cref="Raffa.Quotes.Domain.QuoteLine.Sku"/> holds) — deliberately never pre-normalized by
...
/// <see cref="Raffa.Quotes.Domain.SkuProductMapping.NormalizedSku"/>'s own doc comment: "a lookup
...
/// <see cref="Raffa.Quotes.Domain.SkuProductMapping.NormalizedEdition"/> (informational only, see
...
/// normalized <see cref="Sku"/>, but recorded explicitly — see <see cref="Raffa.Quotes.Domain
