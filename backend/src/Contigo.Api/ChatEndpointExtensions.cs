...
/// dedicated type rather than inline in this file, so <c>Contigo.Api.Tests</c> and
/// <c>Contigo.IntegrationTests</c> can exercise the composition independently of the two thin HTTP
...
/// <c>Contigo.Chat.Application.AskContigoQueryRouter</c> + <c>EmbeddingRetrievalService</c> +
...
/// (<c>Contigo.Chat.Tests.RagAnswerServiceTests</c>) but is no longer this endpoint's own call path
...
public static class ChatEndpointExtensions
{
    ...
    /// `Contigo.ArchitectureTests.DependencyDirectionTests.Host_must_not_contain_domain_types` never
