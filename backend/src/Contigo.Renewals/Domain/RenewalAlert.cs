...
/// <c>renewal.approaching</c> <see cref="Contigo.Renewals.Application.RenewalApproachingEvent"/> —
/// product spec Appendix B's "typical consumer/action" for that event is literally "Create
/// alert/task", and this is that alert. <see cref="Contigo.Renewals.Application.RenewalThresholdScheduler"/>'s
...
/// type, plus <see cref="Contigo.Renewals.Application.RenewalAlertService"/>, is that job.
///
/// <para>
...
/// <see cref="Contigo.Renewals.Infrastructure.Configurations.RenewalAlertConfiguration"/>'s own
...
///
/// <para>
/// Deliberately does not reference <c>Contigo.Documents.Contracts.Domain.Contract</c>: ADR-002's
/// dependency-direction rule only allows this module to reference <c>Contigo.SharedKernel</c> (plus
/// <c>Contigo.Benchmark</c>) — the same reason <see cref="Contigo.Renewals.Application.ContractRenewalTerms"/>
...
public sealed class RenewalAlert : TenantScopedEntity
{
    ...
    /// <see cref="Contigo.Renewals.Application.RenewalApproachingEvent.ContractId"/> when this row
    ...
    /// <see cref="Contigo.Renewals.Application.RenewalApproachingEvent.Milestone"/>.</summary>
    public required RenewalMilestoneKind Milestone { get; set; }

    /// <summary>Which configured <see cref="Contigo.Renewals.Configuration.ThresholdWindowOptions.DaysBeforeDeadline"/>
    /// window matched — echoes <see cref="Contigo.Renewals.Application.RenewalApproachingEvent.ThresholdDays"/>;
    ...
    /// <see cref="Contigo.Renewals.Application.RenewalApproachingEvent.MilestoneDate"/> at creation
    /// time. <see cref="Contigo.Renewals.Application.RenewalAlertService.RecomputeForContractAsync"/>
    /// compares this against a fresh <see cref="Contigo.Renewals.Application.RenewalEngine.Calculate"/>
    ...
    /// <summary>When this row was first raised — <see cref="Contigo.Renewals.Application.RenewalApproachingEvent.OccurredAt"/>
    /// when created from a scheduler tick or a correction-triggered recompute (both go through the
    /// same <see cref="Contigo.Renewals.Application.RenewalThresholdScheduler.EvaluateThresholdsAsync"/>
    ...
    /// correction resolves it (<see cref="Contigo.Renewals.Application.RenewalAlertService.RecomputeForContractAsync"/>
    /// sets this to <see cref="Contigo.SharedKernel.IClock.UtcNow"/> at the moment of resolution).
