...
/// tick (<c>Contigo.Worker.Scheduling.RenewalThresholdSchedulerHostedService</c>, immediately after
...
/// comment) — <c>Contigo.Api.RenewalAlertRecomputeService</c> relies on exactly that when it opens
/// its own scope to re-read the just-corrected <c>Contract</c> row before calling
/// <see cref="RecomputeForContractAsync"/>.
/// </para>
///
/// <para>
/// Depends only on this module's own types plus <see cref="Contigo.SharedKernel"/> — ADR-002 forbids
/// <c>Contigo.Renewals</c> from referencing <c>Contigo.Documents.Contracts</c> at all (the same rule
...
/// real, corrected <c>Contract</c> row onto that shape can only happen in <c>Contigo.Api</c>, "the one
