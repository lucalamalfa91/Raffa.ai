...
- **on-Azure-dev** — observed on `rg-raffa-dev` (2026-09-05 probe)
...
|------|---------|-------------|--------------|---------------|-------|
| Postgres Flexible Server + empty DB | ADR-003/005 | Terraform | `psql-raffa-dev` / `raffa_dev` Ready | designed (`raffa_demo`) | WAVE_COVERED (e01 infra) |
...
| HCP apply vs git | ADR-007 | VCS workspaces | `raffa-dev` behind git | `raffa-demo` | WAVE_COVERED (**e09**) |
...
| Document Intelligence (OCR) on CA | ADR-017 (amended 2026-09-09) | native to `aisvc-raffa`; `prebuilt-read` / 2024-11-30 bound via `AiGateway__Models__Ocr__*` | same gate as above | same gate as above | **E10** → infra 2026-09-09 |
