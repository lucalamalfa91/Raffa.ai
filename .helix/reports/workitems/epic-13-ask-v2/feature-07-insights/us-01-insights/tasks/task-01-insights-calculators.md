...
Fill `backend/src/Raffa.Insights` (scaffolded by F01/T01) with pure,
...
list). (2) Generalize `Raffa.Quotes/Application/Strategy/NegotiationStrategyCalculator`
...
`Raffa.Insights/Contracts/` and referenced by Quotes (Quotes may
reference Insights? — no: Quotes' allow-list is `[SharedKernel,
Benchmark]`; therefore put `PricedLine` in `Raffa.Benchmark/Contracts/`
next to `BenchmarkDistribution`, where both Quotes and Insights can see
it, and keep the calculator's core in `Raffa.Insights/Negotiation/PricedLineNegotiationCalculator`
...
`raffa-v2/app.jsx` `stepDefs`), plus `openWeakFacts[]` and citation
keys for every number (`fact:<contractId>:<field>`, `market:<recordId>`,
`calc:<name>`). (4) `Raffa.Api/InsightsEndpointExtensions.cs` with
...
| Path | Change |
|------|--------|
| `backend/src/Raffa.Insights/Criticality/*`, `Negotiation/*`, `Strategy/*`, `Contracts/*`, `InsightsOptions.cs`, `ServiceCollectionExtensions.cs` | new |
| `backend/src/Raffa.Benchmark/Contracts/PricedLine.cs` | new shared input (only file touched in Benchmark) |
| `backend/src/Raffa.Quotes/Application/Strategy/NegotiationStrategyCalculator.cs` (+ `NegotiationStrategyService.cs` if signatures move) | delegate to the shared core |
| `backend/src/Raffa.Api/InsightsEndpointExtensions.cs` | new, unmapped |
| `backend/tests/Raffa.Insights.Tests/*` | calculators, builder, endpoint composition with fakes |
| `backend/tests/Raffa.Quotes.Tests/*` | unchanged expectations still green |

## Context the implementer needs
- **Architecture decisions in force**: ADR-024 (deterministic strategies; Insights → `[SharedKernel, Benchmark]`), ADR-002 (DTOs, no entity references across modules; composition in `Raffa.Api`), Appendix C rules 6 and 10, spec §9.2, §10.4, §12.1.
- Gap G-STRATEGY, G-CRITICALITY, G-COMPARE. Inputs available today: `Raffa.Renewals.Application.{RenewalEngine,PriorityScoreCalculator,RenewalPipelineBuilder}`, `Raffa.Documents.Contracts.Application.{PortfolioQueryService,Contract360QueryService}`, `Raffa.Savings.Application.SavingsOpportunityService`, `IBenchmarkService`.
- **Do not touch**: `Program.cs` (F05/T02 owns it this phase), `Raffa.Chat`, `DependencyDirectionTests.cs` (F01/T01 added Insights), `web/`.

## Definition of done
- [ ] `dotnet test backend/tests/Raffa.Insights.Tests` exit 0 — components sum to total; same inputs → same order; all-weak contract flagged "validate first"; priced-line targets equal the quote calculator's for identical inputs; builder states a passed deadline; no band → no targets
- [ ] `dotnet test backend/tests/Raffa.Quotes.Tests` exit 0 unchanged
- [ ] `dotnet build backend/Raffa.slnx` exit 0

## Tests required
| Level | What it proves | Where |
|-------|----------------|-------|
| unit | criticality, levers, pack builder, endpoint composition | `Raffa.Insights.Tests/*` |
| unit | quotes unchanged | `Raffa.Quotes.Tests/*` |
