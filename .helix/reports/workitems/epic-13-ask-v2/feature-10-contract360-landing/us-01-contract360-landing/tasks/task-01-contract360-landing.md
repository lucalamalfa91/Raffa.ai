...
`contigo-v2/app.jsx` `hl`, `citedOpened`, `back`, `backLabels`,
`clauses[].show`, `c360Chips`; `contigo-v2/markup.html` evidence block
"{{ hl.before }} **{{ hl.quote }}** {{ hl.after }}" and "← {{ backLabel }}";
`contigo-v2/screens-v2.md` §5 "Why". In
...
Contigo" when `from === "ask"`, else the existing label. Add **Ask about
...
- **Design**: `inputs/design/prototypes/Contigo V2 Prototype.html`; unpacked anchors above; `contigo-v2/styles.css`; requirements R-EVD-02, R-WEB-06 (`inputs/requirements.md`).
...
## Definition of done
- [ ] `npm test` in `web/` exit 0 — `/contracts/:id?clause=<id>` renders the clause highlighted with its `rawText` visible; `?page=12` highlights the first clause on page 12; `state.from === "ask"` → back label "← Ask Contigo"; "Ask about it" links to `/ask?scope=<id>`; header shows `supplierName` when present
