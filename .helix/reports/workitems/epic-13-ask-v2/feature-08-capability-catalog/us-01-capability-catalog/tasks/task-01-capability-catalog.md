...
Teach Contigo its own product (`inputs/requirements.md` R-SYS-01…04). In
`backend/src/Contigo.Chat/Application/Capabilities/`: `Capability` record
...
the V2 design: `inputs/design/prototypes/contigo-v2/ia-v2.md` (route map,
cross-links) and `contigo-v2/app.jsx` `ask()` capabilities branch ("I
answer from your validated contracts and route you to the right part of
Contigo: • Documents — upload contracts, review weak facts. • Portfolio —
...
from validated contracts. Upload one to start." (`contigo-v2/markup.html`).
`FeatureCitation.For(capability)` → citation item (`corpus: contigo`,
...
`Contigo.Api/CapabilitiesEndpointExtensions.cs` — **not mapped** in
...
| Path | Change |
|------|--------|
| `backend/src/Contigo.Chat/Application/Capabilities/Capability.cs`, `CapabilityCatalog.cs`, `CapabilityRouting.cs`, `CopilotAction.cs`, `FeatureCitation.cs`, `RoutingContext.cs` | new |
| `backend/src/Contigo.Chat/Infrastructure/ServiceCollectionExtensions.cs` | register catalog + routing (phase-2 writer) |
| `backend/src/Contigo.Api/CapabilitiesEndpointExtensions.cs` | new, unmapped |
| `backend/tests/Contigo.Chat.Tests/Capabilities/*` | catalog completeness, routing, availability, suggestions |
| `backend/tests/Contigo.Api.Tests/CapabilitiesEndpointTests.cs` | role filtering (uses a test host mapping the endpoint) |

## Context the implementer needs
- **Design**: `inputs/design/prototypes/Contigo V2 Prototype.html`; unpacked `contigo-v2/app.jsx` (`ask()` capabilities branch, `chipsFor`, `c360Chips`), `contigo-v2/markup.html` ("The portfolio lights up from validated contracts."), `contigo-v2/ia-v2.md` (route map + "Ask intents").
...
- Gap G-CAPABILITIES.
- **Do not touch**: `ChatEndpointExtensions.cs`, `AskContigoQueryRouter`, `RagAnswerService` (F06), conversations files (F05), `Program.cs`, `web/`.

## Definition of done
- [ ] `dotnet test backend/tests/Contigo.Chat.Tests --filter Capabilities` exit 0 — every V2 route of `ia-v2.md` has an entry; benchmark intent → Quote check; unknown supplier → Documents; 0 validated contracts → upload action with the prototype copy; hrefs never contain `{`
- [ ] `dotnet test backend/tests/Contigo.Api.Tests --filter CapabilitiesEndpointTests` exit 0 — Procurement does not see `workspace-members`
- [ ] `dotnet build backend/Contigo.slnx` exit 0

## Tests required
| Level | What it proves | Where |
|-------|----------------|-------|
| unit | catalog, routing, availability, suggestions, feature citations | `Contigo.Chat.Tests/Capabilities/*` |
| API | role-aware listing | `Contigo.Api.Tests/CapabilitiesEndpointTests.cs` |
