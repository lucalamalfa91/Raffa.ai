...
`CitationCard` (props: `n`, `corpus` `tenant|market|raffa`, `title`,
`subtitle`, `snippet`, `previewUrl?`, `href?`; renders the corpus badge
*validated contract* / *market · representative* / *Raffa*, the title,
...
from `inputs/design/prototypes/Raffa V2 Prototype.html` — unpacked
`raffa-v2/markup.html` Ask block (`m.who` label column 72px, 14px body,
`white-space: pre-line`, citation chip row → cards, action row with
`btn-secondary`), `raffa-v2/styles.css` (`.tag-*`, `.btn-*`,
`--color-accent-100` fills) and `raffa-v2/screens-v2.md` §2. Never
...
- **Design**: `inputs/design/prototypes/Raffa V2 Prototype.html`; unpacked anchors: `raffa-v2/markup.html` (`sc-for list="{{ gchat }}"` block: `m.hasCites`, `m.hasActions`, `m.abstain`), `raffa-v2/styles.css`, `raffa-v2/screens-v2.md` §2 (answer / abstain / redirect / refusal states). Reply contract: `inputs/requirements.md` §6.
