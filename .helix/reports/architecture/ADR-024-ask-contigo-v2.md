# ADR-024 — Ask Contigo V2: Documents-only intake with admission gate, conversations, market-intelligence feed, capability catalog
...
ADR-023 made Ask Contigo a savings copilot over two corpora but was never
...
improve). Every answer must come from Contigo's own sources, never the web,
...
- Tenant content never leaves the tenant (ADR-009 / ADR-011); market
  knowledge is Contigo-owned and shared, never another tenant's PDF.
...
| **Supplier identity (R-SUP)** | `supplier` becomes a critical extracted fact; `Supplier` entity in `Contigo.Suppliers.Products` (RLS); `ISupplierResolver` port in SharedKernel called from the pipeline orchestration; back-fill through reprocess. Names, never guids, everywhere. |
| **Capability catalog (R-SYS)** | Versioned catalog in `Contigo.Chat` exposed at `GET /api/capabilities`; the planner routes intents to catalog hrefs only (`/contracts/{id}`, `/renewals`, `/savings`, `/documents?…`, `/quotes`); feature citation cards (`corpus: contigo`); availability-aware (greyed modules → upload action). |
| **Foundry (R-AI)** | `FoundryAiGateway` for `ocr` (Document Intelligence), `classify`, `extract`, `embed`, `answer`; registered when `AiGateway:Endpoint` is set (Container Apps inject it), fixture otherwise; always wrapped by `LoggingAiGateway`; Azure SDKs only in `Contigo.AiGateway`; per-tenant daily token / OCR-page budgets fail visibly. |
| **Web V2 IA (R-WEB)** | Pixel and behaviour reference: `Contigo V2 Prototype.html` (unpacked `contigo-v2/`). `/` → `/ask`; `/ask/:conversationId`; two-tier rail (Ask Contigo with recent chats + "+ New chat", Documents; "From your contracts": Portfolio, Renewals, Quote check greyed until the first validated contract; no Home item; Savings at `/savings` from actions); Documents V2 (onboarding empty state, multi-file, per-file outcome incl. **Not added**, attention filter, real stages, review as a state at `/documents?review=`); rich reply; citation landing on the highlighted clause in Contract 360 with "Ask about it" → `/ask?scope=`. Divergences from the prototype are the requirements' (listed in `ia-v2.md`). |
| **Module map (ADR-002 delta)** | New `Contigo.Market` → `[SharedKernel, AiGateway, Benchmark]` (feed, mock, ingestion, index, retrieval, benchmark projection); new `Contigo.Insights` → `[SharedKernel, Benchmark]` (pure calculators fed by DTOs); `Contigo.Suppliers.Products` → `[SharedKernel]` (entity, resolver); `Contigo.Chat` keeps `[SharedKernel, AiGateway]` and gains its own DbContext, gate, planner, guards, prompts, catalog, pack DTOs; composition stays in `Contigo.Api`. Schema lands as checked-in idempotent SQL applied by CI (ADR-021): `chat.sql`, `suppliers.sql`, `market.sql`, regenerated `documents-contracts.sql`. |
...
- **Bad**: twenty tasks across five phases with strict single-writer rules
  on `Program.cs`, `Contigo.slnx`, the OpenAPI contract and the TS client;
...
- Bad: violates D1 and D3, cannot guarantee "verified from Contigo's data",
...
- Every **web** task cites `inputs/design/prototypes/Contigo V2 Prototype.html`
  and an unpacked anchor (`contigo-v2/markup.html` string, `app.jsx`
  symbol, `screens-v2.md` section). Copy is verbatim from the prototype
  unless the requirements override it.
- Single writer per phase for `backend/src/Contigo.Api/Program.cs`,
  `backend/Contigo.slnx`, `DependencyDirectionTests.cs`,
  `.github/workflows/backend.yml` (schema list), `web/openapi/contigo-api.v1.json`,
...
is Contigo's own; thresholds are configuration; `/savings` out of the rail;
