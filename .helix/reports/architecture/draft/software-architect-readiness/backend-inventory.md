...
- OpenAPI is the hand-authored file `web/openapi/contigo-api.v1.json`. The
  API host does **not** publish Swagger UI (correct; out of scope).
- `Contigo.Api` must not call `MigrateAsync()` (ADR-021 / e09).
...
- Benchmark / savings fixture adapter is in-process for tests. There is **no**
  checked-in job that seeds `contigo_demo` (or `contigo_dev`).
