#!/usr/bin/env python3
"""Snapshot / verify that the live plan was not rewritten by visual-fidelity.

Hash-locks e01–e10, wave-spec.execution/web/schema/readiness, ADR-001…022,
epic-01…10. Does NOT lock slice.current.yaml.

Usage (cwd = .helix):
  python scripts/assert_visual_plan_untouched.py snapshot
  python scripts/assert_visual_plan_untouched.py verify
"""
...
SNAP = HERE / "reports" / "plan" / ".visual-protect-snapshot.json"

SLICE_IDS = tuple(f"e{n:02d}" for n in range(1, 11))
WAVE_SPECS = (
...
EPIC_GLOBS = tuple(
    [*(f"epic-0{n}-*" for n in range(1, 10)), "epic-10-*"]
)
...
def snapshot() -> int:
    ...
    payload = {
        ...
        "backlog_must_contain": [*(f"epic-0{n}" for n in range(1, 10)), "epic-10"],
    }
...
def verify() -> int:
    ...
    if broken:
        ...
        return 1
    print("verify: protected plan unchanged (e01-e10, ADR-001..022, epic-01..10)")
    return 0


def main() -> int:
    if len(sys.argv) != 2 or sys.argv[1] not in {"snapshot", "verify"}:
        print("usage: assert_visual_plan_untouched.py snapshot|verify", file=sys.stderr)
        return 2
