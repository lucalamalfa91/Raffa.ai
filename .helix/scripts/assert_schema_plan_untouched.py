#!/usr/bin/env python3
"""Snapshot / verify that the live plan was not rewritten by schema-apply.

Hash-locks e01–e05, wave-spec.execution.yaml, ADR-001…020, epic-01…08.
Does NOT lock slice.current.yaml.

Usage (cwd = .helix):
  python scripts/assert_schema_plan_untouched.py snapshot
  python scripts/assert_schema_plan_untouched.py verify
"""
...
SNAP = HERE / "reports" / "plan" / ".schema-protect-snapshot.json"

SLICE_IDS = ("e01", "e02", "e03", "e04", "e05")
EPIC_GLOBS = tuple(f"epic-0{n}-*" for n in range(1, 9))
...
def snapshot() -> int:
    ...
    payload = {
        ...
        "index_must_contain": [f"ADR-{n:03d}" for n in range(1, 21)],
        "backlog_must_contain": [f"epic-0{n}" for n in range(1, 9)],
    }
...
def verify() -> int:
    ...
    if broken:
        ...
        return 1
    print("verify: protected plan unchanged (e01-e05, ADR-001..020, epic-01..08)")
    return 0


def main() -> int:
    if len(sys.argv) != 2 or sys.argv[1] not in {"snapshot", "verify"}:
        print("usage: assert_schema_plan_untouched.py snapshot|verify", file=sys.stderr)
        return 2
