...
class Component extends DCLogic {
  ...
  renderVals(){
    ...
    // pilot acts chrome
    const live=s.docs[0]; const validated=s.docs.some(d=>d.status==='completed'); const answered=s.chat.some(m=>m.who==='Contigo');
    ...
    const backLabels={ask:'Ask Contigo',documents:'Documents',portfolio:'Portfolio',renewals:'Renewals',c360:'Ask Contigo'};
    ...
    return {
      ...
      showNewChat:scr.ask&&!!activeCv, askHello:'What do you want to know?', askBarDot:kbReady?'var(--color-accent)':'var(--color-neutral-400)', askChips:kbReady?askChips:[], askPlaceholder:kbReady?'Ask Contigo — spend, dates, clauses, liability…':'Ask Contigo switches on after your first validated contract',
      ...
      convs, noConvs:!s.convs.length, hasActiveConv:!!activeCv, convTitle:activeCv?activeCv.title:'Ask Contigo · new chat',
      ...
      rdoc, reviewTitle:openWeak?openWeak+' fact'+(openWeak>1?'s':'')+' below 80% — you decide':'All weak facts decided', reviewSub:openWeak?'Contigo will not use these for renewals or Ask until you accept or correct them. Everything else was extracted above threshold.':'Mark the contract as validated to make it askable.',
      ...
      askOffReason:docs.length?'Your document is still processing or waiting for review. Ask only answers from facts that passed validation — so it never guesses.':'Upload a contract first. Contigo extracts the facts, you sign off the weak ones, and Ask switches on.', askOffCta:docs.length?'Go to Documents':'Upload a contract',
  ...
  ask(text,scope){
    ...
    if(any('benchmark','compar','confront','competitor','concorrent','in linea','allineat','market','mercato','fair price','prezzo giusto','too much','troppo')){
      ...
      a=has(1)?{text:'The largest identified saving is the Salesforce renewal: CHF 80–120k per year (you pay CHF 156/user vs a market median of 132). The price-uplift clause behind it was extracted at 64% confidence and needs your sign-off before Contigo counts it.',route:'Benchmark match on validated unit price · adapter A, n = 214',cites:[{ev:'sf_uplift',cid:1,tab:'Clauses'}]}:{abstain:true,text:'',reason:'No validated contract has matched a benchmark yet.'};
    } else {
    ...
    this.later(()=>this.setState(st=>({asking:false,chat:[...st.chat,Object.assign({who:'Contigo',scope,conv},a)]})),1000);
  }
}
