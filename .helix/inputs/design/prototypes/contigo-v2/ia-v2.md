# Contigo — Information architecture (web V2, pilot path)

Source: `Contigo V2 Prototype.html` (unpacked here as `app.jsx` +
...
1. **Ask Contigo** — badge `⌘K`; the last 5 conversations nested under it
...
| `/ask` | Ask Contigo — new chat (home) | Conversation | `scr.ask`, `chatEmpty`, `askHello` |
| `/ask/:conversationId` | Ask Contigo — resumed conversation | Conversation | `convs[].resume`, `convTitle` |
...
- Ask citation card → Contract 360 with the clause highlighted (`hl`,
  `citedOpened`), back label "Ask Contigo".
