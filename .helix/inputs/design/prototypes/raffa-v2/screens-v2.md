# Raffa — Screens (web V2, pilot path)

Pixel reference: `Raffa V2 Prototype.html` (unpacked: `markup.html`,
...
## 2. Ask Raffa — home (R1 §8.3–§8.4; requirements §5.2–§5.7)

- **Off** (no validated contract): "Ask needs at least one validated
  contract." + `askOffReason` ("Upload a contract first. Raffa extracts the
...
- **New chat**: "What do you want to know?" (`askHello`) + scope line
  ...
  cannot answer." Input placeholder "Ask Raffa — spend, dates, clauses,
  liability…"; two suggestion chips.
- **Conversation**: header `convTitle` with **+ New chat**; turns You (17px
  heading weight) / Raffa (14px); Raffa turns render *req* markdown
...
- Rail: last 5 conversations under Ask Raffa, resume by click, active one
  in accent.
...
decide" / "All weak facts decided"); sub "Raffa will not use these for
...
80–95% flag, <80% you decide. Below 80% Raffa will not use the fact for
...
Raffa / Documents / Portfolio / Renewals).
...
Empty: "Drop a supplier proposal; Raffa normalises the lines and compares
