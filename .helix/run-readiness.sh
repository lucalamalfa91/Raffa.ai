#!/usr/bin/env bash
# Launch the DEMO-READINESS process only. Refuses --fresh and --slice.
set -euo pipefail
...
for a in "$@"; do
  case "$a" in
    --check|-Check) CHECK=1 ;;
    --fresh|-Fresh)
      echo "ERROR: run-readiness.sh refuses --fresh" >&2
      exit 1
      ;;
    --slice|-Slice|--slice=*|--max|-Max)
      echo "ERROR: run-readiness.sh has no fan-out. After gap-report HITL + e09 + e06-e08:" >&2
      echo "  ./run.ps1 -Max -Slice e10 -o execution-fanout" >&2
      exit 1
      ;;
    *) ARGS+=("$a") ;;
  esac
