...
implications, §9 gap analysis), `inputs/design/prototypes/contigo-v2/ia-v2.md`
and `screens-v2.md`, ADR-001, ADR-004, ADR-011, ADR-018, ADR-020, ADR-023,
`backend/src/Contigo.Api/ChatEndpointExtensions.cs`,
`backend/src/Contigo.Api/Program.cs` (the `/api/documents` block),
`backend/src/Contigo.AiGateway/Fixtures/FixtureAiGateway.cs`,
`backend/src/Contigo.Documents.Contracts/Application/Extraction/DocumentProcessingPipeline.cs`,
...
1. `reports/audit/ask-v2-gaps.md` — OPEN / WAVE_COVERED / DEFERRED rows.
   ...
   (`contigo-v2/markup.html` string or `app.jsx` symbol).
2. `reports/architecture/ADR-024-ask-contigo-v2.md` — new, **accepted**,
   ...
   pixel reference, module map deltas (`Contigo.Market`, `Contigo.Insights`,
   `Contigo.Suppliers.Products`, Chat DbContext).
...
5. Append the "Ask Contigo V2 (wave 13 / e13)" section to
